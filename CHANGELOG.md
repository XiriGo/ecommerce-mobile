...
#### Design Quality Backfill (DQ-35)

- **HomeScreen LazyVStack/LazyHStack refactor**: Replaced eager `VStack` with `LazyVStack` inside `ScrollView` in iOS `HomeScreen.swift` for on-demand rendering of vertical sections. Replaced `HStack` with `LazyHStack` in categories and popular products horizontal scroll sections in `HomeScreenSections.swift`. New arrivals grid already used `LazyVGrid` and required no changes. Pull-to-refresh, banner auto-scroll, and all existing behavior preserved. No domain, data, or ViewModel changes. Existing ViewModel tests verified. (#79) (iOS)
