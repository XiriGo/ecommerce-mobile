...
struct SessionManagerTests {
    ...
    func login_success_storesTokenAndSetsAuthenticatedState() async throws {
    ...
    func login_failure_throwsErrorAndDoesNotUpdateState() async {
        MockURLProtocol.reset()
    ...
    func login_networkError_throwsNetworkError() async {
        MockURLProtocol.reset()
    ...
    func register_success_storesTokenAndSetsAuthenticatedState() async throws {
        MockURLProtocol.reset()
        var callCount = 0
        MockURLProtocol.requestHandler = { request in
            callCount += 1
    ...
    func register_emailExists_throwsServerError() async {
        MockURLProtocol.reset()
    ...
    func logout_apiSucceeds_clearsLocalState() async {
        MockURLProtocol.reset()
    ...
    func logout_apiFails_stillClearsLocalState() async {
        MockURLProtocol.reset()
        MockURLProtocol.stubNetworkError(.notConnectedToInternet)
    ...
    func logout_server500_stillClearsLocalState() async {
        MockURLProtocol.reset()
    ...
    func refreshToken_success_storesNewToken() async throws {
        MockURLProtocol.reset()
    ...
    func refreshToken_failure_throwsError() async {
        MockURLProtocol.reset()
        MockURLProtocol.stub(
            statusCode: 401,
    ...
    func getAccessToken_returnsTokenFromStorage() async {
        MockURLProtocol.reset()
        let storage = FakeTokenStorage()
    ...
    func getAccessToken_emptyStorage_returnsNil() async {
        MockURLProtocol.reset()
    ...
    func clearTokens_setsGuestStateAndClearsStorage() async {
        MockURLProtocol.reset()
        let storage = FakeTokenStorage()
