...
struct XGQuantityStepperTests {
    ...
    func init_withBindingAndHandler_initialises() {
        var quantity = 3
        let binding = Binding(get: { quantity }, set: { quantity = $0 })
        let stepper = XGQuantityStepper(
            quantity: binding,
    ...
    func init_defaultMinQuantity_is1() {
        var quantity = 1
        let binding = Binding(get: { quantity }, set: { quantity = $0 })
        let stepper = XGQuantityStepper(
            quantity: binding,
    ...
    func init_defaultMaxQuantity_is99() {
        var quantity = 1
        let binding = Binding(get: { quantity }, set: { quantity = $0 })
        let stepper = XGQuantityStepper(
            quantity: binding,
    ...
    func init_customMinMax_initialises() {
        var quantity = 5
        let binding = Binding(get: { quantity }, set: { quantity = $0 })
        let stepper = XGQuantityStepper(
            quantity: binding,
    ...
    func canDecrease_atCustomMin_isFalse() {
    ...
    func canIncrease_atCustomMax_isFalse() {
    ...
    func decrease_aboveMin_decrementsBy1() {
        var quantity = 5
    ...
    func decrease_atMin_doesNotChange() {
        var quantity = 1
        decrease(quantity: &quantity, min: 1, max: 99)
        #expect(quantity == 1)
    }
    ...
    func decrease_firesCallback_withNewValue() {
        var quantity = 5
    ...
    func increase_belowMax_incrementsBy1() {
        var quantity = 3
    ...
    func increase_atMax_doesNotChange() {
        var quantity = 99
        increase(quantity: &quantity, min: 1, max: 99)
        #expect(quantity == 99)
    }
    ...
    func increase_firesCallback_withNewValue() {
        var quantity = 3
        var callbackValue: Int?
    ...
    func boundary_minBound_staysAtMin() {
        var quantity = 1
        for _ in 0 ..< 10 {
            decrease(quantity: &quantity, min: 1, max: 99)
        }
    ...
    func boundary_maxBound_staysAtMax() {
        var quantity = 99
        for _ in 0 ..< 10 {
            increase(quantity: &quantity, min: 1, max: 99)
        }
    ...
    func increase_from1ToMax_quantity99() {
        var quantity = 1
        for _ in 0 ..< 98 {
            increase(quantity: &quantity, min: 1, max: 99)
        }
