    ...
    private static let sourceRoot: URL = {
        ...
        if let srcRoot = ProcessInfo.processInfo.environment["SRCROOT"] {
            return URL(fileURLWithPath: srcRoot)
                .appendingPathComponent("XiriGoEcommerce")
        }

        // Fallback: walk up from current file location to find the project
        let url = URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent() // ArchitectureTests.swift -> XiriGoEcommerceTests/
        let projectRoot = url.deletingLastPathComponent() // XiriGoEcommerceTests/ -> ios/
        return projectRoot.appendingPathComponent("XiriGoEcommerce")
    }()
