...
enum DeepLinkParser {
    ...
    private static func parseXGScheme(host: String, pathComponents: [String]) -> Route? {
        switch host {
            case "product":
                parseProductRoute(pathComponents: pathComponents)

            case "category":
                parseCategoryRoute(pathComponents: pathComponents)

            case "cart":
                .cart

            case "order":
                parseOrderRoute(pathComponents: pathComponents)
    ...
    private static func parseProductRoute(pathComponents: [String]) -> Route? {
        guard let productId = pathComponents.first, !productId.isEmpty else {
            return nil
        }
    ...
    private static func parseCategoryRoute(pathComponents: [String]) -> Route? {
        guard let categoryId = pathComponents.first, !categoryId.isEmpty else {
            return nil
        }
    ...
    private static func parseOrderRoute(pathComponents: [String]) -> Route? {
        guard let orderId = pathComponents.first, !orderId.isEmpty else {
            return nil
        }
