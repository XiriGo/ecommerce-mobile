...
## Feature
**M0-01: App Scaffold** -- Foundation project structure for XiriGo Ecommerce buyer app.
...
- `XGApplication` (`@HiltAndroidApp`) + `MainActivity` (`@AndroidEntryPoint`, splash screen)
- Design system theme shell: `XGColors`, `XGTypography`, `XGSpacing`, `XGTheme`
...
- `XiriGoEcommerceApp` (`@main` SwiftUI App entry point) + `Config.swift`
- Design system theme shell: `XGColors`, `XGTypography`, `XGSpacing`, `XGTheme`
...
- Environment URLs: dev (`api-dev.xirigo.com`), staging (`api-staging.xirigo.com`), prod (`api.xirigo.com`)
- Splash screen: surface background, placeholder logo, no artificial delay
- Placeholder screen: centered "XiriGo Ecommerce" text using `headlineSmall` style
...
3. **Theme shell in scaffold**: `XGColors`, `XGTypography`, `XGSpacing`, `XGTheme` created here so M0-02 components can reference them immediately
...
- **M0-02 (Design System)**: Implement `XG*` components inside `core/designsystem/component/`. Theme files already exist.
- **M0-03 (Network Layer)**: Implement inside `core/network/`. Base URL is already configured per environment.
- **M0-04 (Navigation)**: Replace placeholder screen in `MainActivity`/`XiriGoEcommerceApp` with tab bar + navigation.
